...
// These perk texts are product names/features — keep hardcoded as they are brand-specific
const PERK_TEXTS = [
...
function getLevel(count, t) {
  const labels = [
    t.ambassadorNewcomer,
    t.ambassadorRisingStar,
    t.ambassadorCommunityBuilder,
    t.ambassadorCityLegend,
    t.ambassadorVybtIcon,
    t.ambassadorElite,
  ];
  const colors = [
  ...
  if (count < 10) return { level: 0, label: labels[0], next: 10, color: colors[0] };
  const lvl = Math.floor(count / 10);
  const capped = Math.min(lvl, 5);
  return {
    level: capped,
    label: labels[capped] || labels[5],
...
export default function Ambassador() {
    ...
    const load = async () => {
      try {
        ...
        if (p && !p.referral_code) {
          ...
          await base44.entities.UserProfile.update(p.id, { referral_code: code, ambassador_opted_in: true });
  ...
  const handleCopy = () => {
    ...
    setCopied(true);
    toast.success(t.ambassadorLinkCopied);
  ...
  const handleShare = async () => {
    if (!referralLink) return;
    if (navigator.share) {
      await navigator.share({ title: t.ambassadorShareTitle, text: t.ambassadorShareText, url: referralLink });
    } else {
      handleCopy();
    }
  };
  ...
  const levelInfo = getLevel(inviteCount, t);
  const progressToNext = levelInfo.next ? Math.min(((inviteCount % 10) / 10) * 100, 100) : 100;
  ...
  return (
    <div className="min-h-screen pb-10" style={{ background: 'var(--bg)' }}>
      {/* Header */}
      <div className="sticky top-0 z-10 backdrop-blur-lg border-b border-gray-800 px-5 py-4 flex items-center gap-3"
        style={{ background: 'var(--bg)', opacity: 0.95 }}>
        <motion.button whileTap={{ scale: 0.9 }} onClick={() => navigate(-1)}>
          <ChevronLeft className="w-6 h-6 text-white" />
        </motion.button>
        <h1 className="text-white font-bold text-lg flex-1">{t.ambassadorTitle}</h1>
        {isAmbassador && (
      ...
      <div className="px-5 py-6 space-y-6">
        ...
        <div className="text-center">
          ...
          {isAmbassador ? (
            <>
              <h2 className="text-2xl font-bold text-white mb-1">{t.ambassadorYouAre}</h2>
              <p className="text-purple-400 text-sm">
                {t.ambassadorLevel.replace('{level}', levelInfo.level).replace('{label}', levelInfo.label)}
              </p>
            </>
          ) : (
            <>
              <h2 className="text-2xl font-bold text-white mb-1">{t.ambassadorBecome}</h2>
              <p className="text-gray-400 text-sm">{t.ambassadorBecomeDesc}</p>
            </>
          )}
        </div>
          ...
          <div className="flex items-center justify-between mb-1">
            <div className="flex items-center gap-2">
              ...
              <span className="text-white font-semibold">
                {t.ambassadorLevelLabel.replace('{level}', levelInfo.level).replace('{label}', levelInfo.label)}
              </span>
            </div>
            <span className="text-purple-400 font-bold text-lg">
              {t.ambassadorInvited.replace('{count}', inviteCount)}
            </span>
          </div>
          {levelInfo.next && (
            <p className="text-gray-500 text-xs mb-3">
              {t.ambassadorMoreToLevel.replace('{count}', levelInfo.next - inviteCount).replace('{level}', levelInfo.level + 1)}
            </p>
          )}
          {!levelInfo.next && (
            <p className="text-[#00c6d2] text-xs mb-3 font-semibold">{t.ambassadorMaxLevel}</p>
          )}
        ...
        <div className="space-y-2">
          <h3 className="text-white font-semibold text-sm uppercase tracking-widest">{t.ambassadorMilestones}</h3>
          <div className="grid grid-cols-5 gap-2">
            {milestones.map((m) => {
              const reached = inviteCount >= m;
              const levelIdx = Math.floor(m / 10);
              const colors = [
                'from-purple-600 to-purple-400', 'from-blue-600 to-blue-400',
                'from-[#00c6d2] to-teal-400', 'from-yellow-500 to-amber-400', 'from-pink-500 to-rose-400',
              ];
              ...
              return (
                <div key={m} className="flex flex-col items-center gap-1">
                  ...
                  <span className={`text-[10px] text-center ${reached ? 'text-white' : 'text-gray-600'}`}>{m} {t.ambassadorInvAbbr}</span>
                </div>
              );
            })}
          </div>
        ...
        <div className={`rounded-2xl p-5 border ${isAmbassador ? 'bg-[#5865F2]/10 border-[#5865F2]/40' : 'bg-gray-900 border-gray-800 opacity-60'}`}>
          <div className="flex items-center gap-3 mb-3">
            ...
            <div>
              <h3 className="text-white font-semibold">{t.ambassadorDiscordTitle}</h3>
              <p className="text-gray-400 text-xs">
                {isAmbassador ? t.ambassadorDiscordUnlocked : t.ambassadorDiscordLocked}
              </p>
            </div>
            {!isAmbassador && <Lock className="w-4 h-4 text-gray-600 ml-auto flex-shrink-0" />}
          </div>
          {isAmbassador ? (
            <motion.a href={DISCORD_LINK} target="_blank" rel="noopener noreferrer"
              whileTap={{ scale: 0.97 }} whileHover={{ scale: 1.02 }}
              className="w-full py-3 rounded-2xl bg-[#5865F2] text-white font-bold flex items-center justify-center gap-2">
              <ExternalLink className="w-4 h-4" />
              {t.ambassadorJoinDiscord}
            </motion.a>
          ) : (
            <div className="w-full py-3 rounded-2xl bg-gray-800 text-gray-500 font-bold flex items-center justify-center gap-2 text-sm">
              ...
              {t.ambassadorInviteMore.replace('{count}', 10 - inviteCount)}
            </div>
          )}
        </div>
        ...
        {referredUsers.length > 0 && (
          <div className="bg-gray-900 rounded-2xl p-5 border border-gray-800">
            <h3 className="text-white font-semibold mb-3 flex items-center gap-2">
              ...
              {t.ambassadorFriendsTitle.replace('{count}', referredUsers.length)}
            </h3>
            <div className="space-y-2">
              {referredUsers.map((u) => (
                <div key={u.id} className="flex items-center gap-3">
                  ...
                  <span className="text-gray-300 text-sm">{u.display_name || t.user}</span>
          ...
          <div className="space-y-2.5">
            {ambassadorPerks.map((perk, i) => (
              <div key={i} className={`flex items-center gap-3 ${isAmbassador ? 'opacity-100' : i < 3 ? 'opacity-100' : 'opacity-50'}`}>
                ...
                <span className="text-gray-300 text-sm">{PERK_TEXTS[i]}</span>
