...
export default function Chat() {
  ...
  useEffect(() => {
    if (groupPlanId) navigate(createPageUrl('GroupChat') + `?planId=${groupPlanId}`, { replace: true });
  }, [groupPlanId]);
  ...
  useEffect(() => {
    ...
    const unsub = base44.entities.ChatMessage.subscribe((event) => {
      if (event.data?.message_type === 'direct') queryClient.invalidateQueries(['allDMMessages', currentUser.id]);
      else if (event.data?.message_type === 'group') queryClient.invalidateQueries(['allGroupMessages', currentUser.id]);
    });
    return () => unsub();
  ...
  if (selectedFriendId) {
    return (
      <div className="flex flex-col overflow-hidden" style={{ position: 'fixed', inset: 0, overflow: 'hidden', background: 'var(--bg)' }}>
        {/* Header */}
          ...
          <motion.button whileTap={{ scale: 0.95 }}
            ...
            <div className="relative flex-shrink-0">
              ...
              {(() => {
                ...
                return isOnline ? <div className="absolute bottom-0 right-0 w-2.5 h-2.5 bg-green-400 rounded-full border-2 border-[#0b0b0b]" /> : null;
              })()}
            </div>
            <div className="text-left min-w-0">
              <p className="text-white font-semibold text-sm truncate">{selectedFriendProfile?.display_name || t.user}</p>
              {selectedFriendProfile?.city && <p className="text-[#00c6d2] text-xs truncate">📍 {selectedFriendProfile.city}</p>}
            </div>
          </motion.button>
          ...
          <div className="relative flex-shrink-0">
            ...
            {showChatMenu && (
              <>
                ...
                <div className="absolute right-0 top-11 z-50 w-52 rounded-2xl border border-gray-700/50 shadow-2xl overflow-hidden" style={{ background: 'var(--bg)' }}>
                  <button onClick={() => { setShowClearConfirm(true); setShowChatMenu(false); }}
                  ...
                  <button onClick={() => { navigate(createPageUrl('UserProfile') + `?id=${selectedFriendId}&report=1`); setShowChatMenu(false); }}
                  ...
                  <div className="h-px bg-gray-800" />
                  <button onClick={() => { navigate(createPageUrl('UserProfile') + `?id=${selectedFriendId}&block=1`); setShowChatMenu(false); }}
                    className="w-full flex items-center gap-3 px-4 py-3.5 text-sm text-red-400 hover:bg-white/5 transition-colors">
                    <ShieldOff className="w-4 h-4" />{t.blockUser2}
                  </button>
                </div>
              </>
            )}
          ...
          <div className="fixed inset-0 z-50 flex items-end justify-center" onClick={() => setShowClearConfirm(false)}>
            ...
            <motion.div initial={{ y: 100, opacity: 0 }} animate={{ y: 0, opacity: 1 }}
              ...
              <div className="flex items-center gap-3 mb-2">
                ...
                <h3 className="text-white font-bold text-lg">{t.clearChat}</h3>
              </div>
              <p className="text-gray-400 text-sm mb-6 ml-13">{t.clearChatDesc}</p>
              <div className="flex gap-3">
                <button onClick={() => setShowClearConfirm(false)}
                  className="flex-1 py-3 rounded-2xl bg-gray-800 text-gray-300 font-semibold text-sm">{t.cancel}</button>
                <button onClick={() => clearChatMutation.mutate()} disabled={clearChatMutation.isPending}
                  ...
                  {clearChatMutation.isPending ? <Loader2 className="w-4 h-4 animate-spin" /> : t.deleteAll}
                </button>
              </div>
            </motion.div>
          </div>
          ...
          {dmLoading ? (
            ...
            <div className="flex flex-col items-center justify-center h-full gap-4 opacity-60">
              ...
              <div className="text-center">
                <p className="text-gray-400 font-medium text-sm">{t.startConversationWith}</p>
                <p className="text-[#00c6d2] font-bold">{selectedFriendProfile?.display_name || t.user} 👋</p>
              </div>
            </div>
          ) : (
            sortedDMs.map((msg) => (
              <ChatMessage key={msg.id} message={msg} isMe={msg.sender_id === currentUser?.id}
        ...
        <div className="absolute bottom-0 left-0 right-0 z-20 border-t border-gray-800/50 backdrop-blur-xl" style={{ background: 'var(--bg)' }}>
          <StickerPicker isOpen={showStickers} onClose={() => setShowStickers(false)}
            onSelect={(sticker) => { sendDMMutation.mutate(`sticker:${sticker.image_url}`); setShowStickers(false); }}
          ...
          <div className="flex items-center gap-2 px-3 py-3" style={{ paddingBottom: 'max(env(safe-area-inset-bottom, 0px), 12px)' }}>
            ...
            <input value={newMessage} onChange={(e) => setNewMessage(e.target.value)}
              ...
              style={{ fontSize: '16px' }}
              onKeyDown={(e) => { if (e.key === 'Enter' && newMessage.trim()) sendDMMutation.mutate(newMessage); }} />
            <motion.button whileTap={{ scale: 0.9 }} onClick={() => newMessage.trim() && sendDMMutation.mutate(newMessage)}
    ...
    <div className="overflow-hidden flex flex-col" style={{ position: 'fixed', inset: 0, background: 'var(--bg)' }}>
      <header className="flex-shrink-0 z-40" style={{ paddingTop: 'env(safe-area-inset-top)', background: 'var(--bg)' }}>
        <div className="px-4 pt-4 pb-2 flex items-center justify-between">
          <div>
            <h1 className="text-2xl font-bold" style={{ color: 'var(--text-primary)' }}>{t.messagesTitle}</h1>
            <p className="text-xs text-gray-500 mt-0.5">
              {totalUnreadDMs + totalUnreadGroups > 0
                ? <span className="text-[#00c6d2] font-medium">{t.unreadMessages.replace('{count}', totalUnreadDMs + totalUnreadGroups)}</span>
                : t.allRead}
            </p>
          </div>
        </div>
        ...
        <div className="flex gap-2 px-4 pt-2 pb-3">
          {[
            { key: 'groups', label: t.tabGroups, count: totalUnreadGroups },
            { key: 'direct', label: t.tabDirects, count: totalUnreadDMs },
          ].map(({ key, label, count }) => (
            <motion.button key={key} whileTap={{ scale: 0.95 }} onClick={() => setActiveTab(key)}
              className={`flex-1 py-2.5 rounded-2xl text-sm font-semibold transition-all relative overflow-hidden ${
                activeTab === key ? 'text-[var(--secondary-text)]' : 'bg-[var(--bg-secondary)] text-[var(--secondary-text)] border border-gray-800'
              }`}
        ...
        {activeTab === 'groups' ? (
          myPlans.length > 0 ? (
                      ...
                      <div className="w-full h-full rounded-xl overflow-hidden">
                        {plan.group_image || plan.cover_image ? (
                          ...
                          <div className="w-full h-full flex items-center justify-center text-2xl"
                            style={{ background: `linear-gradient(135deg, ${color}60, var(--bg-secondary))` }}>🎉</div>
                        )}
                      </div>
                      {groupUnread > 0 && (
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      {lastGroupMsg ? (
                        <p className={`text-xs truncate ${groupUnread > 0 ? 'text-gray-300 font-medium' : 'text-gray-500'}`}>
                          {getMsgPreview(lastGroupMsg.content)}
                        </p>
                      ) : (
                        <div className="flex gap-1 mt-0.5">
                          {plan.tags?.slice(0, 2).map((tag, i) => <PartyTag key={i} tag={tag} size="sm" />)}
                        </div>
                      )}
                    </div>
                  </div>
            ...
            <div className="flex flex-col items-center justify-center py-20 gap-4 opacity-50">
              ...
              <p className="text-gray-500 text-sm">{t.joinPlanForGroups}</p>
            </div>
          )
        ) : (
          friendships.length > 0 ? (
                ...
                <motion.button key={f.id} initial={{ opacity: 0, x: -12 }} animate={{ opacity: 1, x: 0 }}
                  ...
                  <div className="relative flex-shrink-0">
                    ...
                    {(() => {
                      ...
                      const isOnline = ls && (Date.now() - new Date(ls).getTime()) < 5 * 60 * 1000;
                      return isOnline ? (
                        <div className="absolute bottom-0.5 right-0.5 w-3.5 h-3.5 rounded-full flex items-center justify-center" style={{ background: 'var(--bg)' }}>
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center justify-between gap-2 mt-0.5">
                      <p className={`text-[13px] truncate ${unreadCount > 0 ? 'text-gray-200 font-medium' : 'text-gray-500'}`}>
                        {previewText
                          ? <>{isLastMine && <span className="text-[#00c6d2] font-semibold">{t.youPrefix}</span>}{previewText}</>
                          : <span className="italic text-gray-600">{t.startConversationPrompt}</span>
                        }
                      </p>
                      {unreadCount > 0 && (
            ...
            <div className="flex flex-col items-center justify-center py-20 gap-4 opacity-50">
              ...
              <p className="text-gray-500 text-sm">{t.addFriendsToChat}</p>
            </div>
          )
        )}
      </main>
