    ...
    <AnimatePresence>
      <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-4">
        ...
        <motion.div initial={{ opacity: 0, y: 50 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 50 }}
          ...
          <button onClick={onClose} className="absolute top-4 right-4 p-2 rounded-full bg-gray-800 hover:bg-gray-700 z-10">
            <X className="w-5 h-5 text-gray-400" />
          </button>

          <h2 className="text-2xl font-bold text-white mb-2">{t.editPlanTitle}</h2>
          {locked && (
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div>
              <label className="block text-gray-400 text-sm mb-2">
                <ImageIcon className="w-4 h-4 inline mr-2" />{t.planCover}
              </label>
                ...
                {formData.cover_image ? (
                  <div className="relative h-32 rounded-xl overflow-hidden">
                    ...
                    <div className="absolute inset-0 bg-black/50 flex items-center justify-center opacity-0 hover:opacity-100 transition-opacity">
                      {uploading ? <Loader2 className="w-6 h-6 text-white animate-spin" /> : <span className="text-white text-sm">{t.changeCover}</span>}
                    </div>
                  </div>
                ) : (
                  <div className="h-32 rounded-xl border-2 border-dashed border-gray-700 flex items-center justify-center">
                    {uploading ? <Loader2 className="w-6 h-6 text-gray-500 animate-spin" /> : <span className="text-gray-500 text-sm">{t.addCover}</span>}
                  </div>
                )}
            ...
            <div>
              <label className="block text-gray-400 text-sm mb-2">
                {t.planName} {locked && <span className="text-orange-400 text-xs ml-1">🔒 {t.locked}</span>}
              </label>
              <Input
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="block text-gray-400 text-sm mb-2">
                  <Clock className="w-4 h-4 inline mr-1" />{t.startTime2} {locked && <span className="text-orange-400 text-xs">🔒</span>}
                </label>
              ...
              <div>
                <label className="block text-gray-400 text-sm mb-2">
                  <Clock className="w-4 h-4 inline mr-1" />{t.endTime2}
                </label>
            ...
            <div>
              <label className="block text-gray-400 text-sm mb-2">
                <MapPin className="w-4 h-4 inline mr-1" />{t.address} {locked && <span className="text-orange-400 text-xs ml-1">🔒 {t.locked}</span>}
              </label>
              {locked ? (
                <Input value={formData.location_address} readOnly className="bg-gray-800/40 border-gray-700 text-white opacity-50 cursor-not-allowed" />
              ) : (
                <>
                  <AddressAutocomplete
                  ...
                  {formData.city && (
                    <div className="mt-2">
                      <label className="block text-gray-500 text-xs mb-1">{t.city}</label>
            ...
            <div>
              <label className="block text-gray-400 text-sm mb-2">
                <Palette className="w-4 h-4 inline mr-1" />{t.groupColor}
              </label>
              <div className="flex gap-2 flex-wrap">
                {themeColors.map((color) => (
                  <button key={color} type="button" onClick={() => setFormData({ ...formData, theme_color: color })}
                    className={`w-8 h-8 rounded-lg transition-all ${formData.theme_color === color ? 'ring-2 ring-white ring-offset-2 ring-offset-gray-900' : ''}`}
            ...
            {/* Chat Background */}
            <div>
              <label className="block text-gray-400 text-sm mb-3">{t.chatBackground}</label>
              <div className="flex flex-wrap gap-3">
                {Object.entries(GROUP_CHAT_THEMES).map(([key, theme]) => (
                  <motion.button key={key} type="button" whileTap={{ scale: 0.92 }}
                    onClick={() => setFormData({ ...formData, chat_background_theme: key })}
                    className="relative flex flex-col items-center gap-1.5">
                    <div className={`w-12 h-12 rounded-2xl border-2 transition-all flex items-center justify-center ${
                      formData.chat_background_theme === key ? 'border-[#00c6d2] ring-2 ring-[#00c6d2]/40' : 'border-gray-700'
                    }`} style={{ background: `linear-gradient(135deg, ${theme.previewColor} 0%, ${theme.previewAccent} 100%)` }}>
                      {theme.emoji ? <span className="text-lg">{theme.emoji}</span> : <span className="text-xs text-gray-500">—</span>}
            ...
            <div>
              <div className="flex items-center justify-between mb-2">
                <label className="text-gray-400 text-sm">
                  {t.partyTagsMax} {locked && <span className="text-orange-400 text-xs ml-1">🔒 {t.locked}</span>}
                </label>
              ...
              <div className={`flex flex-wrap gap-2 max-h-40 overflow-y-auto ${locked ? 'opacity-50 pointer-events-none' : ''}`}>
                {ALL_PARTY_TYPES.filter(tag => tag.toLowerCase().includes(tagSearch.toLowerCase())).map((tag) => (
                  <PartyTag key={tag} tag={tag} size="sm" interactive={!locked}
                    selected={formData.tags.includes(tag)} onClick={() => toggleTag(tag)} />
                ))}
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="block text-gray-400 text-sm mb-2">{t.entryPrice}</label>
                <div className="relative">
                  <span className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-500 text-sm">€</span>
                  <Input type="number" min="0" step="0.5" value={formData.price ?? ''}
                    onChange={(e) => setFormData(prev => ({ ...prev, price: e.target.value === '' ? undefined : Number(e.target.value) }))}
                    placeholder={t.freePrice} className="bg-gray-800 border-gray-700 text-white pl-7" />
                </div>
              </div>
              <div>
                <label className="block text-gray-400 text-sm mb-2">{t.minAge}</label>
                <Input type="number" min="0" max="100" value={formData.min_age ?? ''}
                  onChange={(e) => setFormData(prev => ({ ...prev, min_age: e.target.value === '' ? undefined : Number(e.target.value) }))}
                  placeholder="e.g. 18" className="bg-gray-800 border-gray-700 text-white" />
              </div>
            </div>

            {/* Privacy */}
            <div>
              <label className="block text-gray-400 text-sm mb-3 font-medium">{t.privacyVisibility}</label>
              <PlanPrivacySettings data={formData} onChange={(updates) => setFormData(prev => ({ ...prev, ...updates }))} />
            </div>
