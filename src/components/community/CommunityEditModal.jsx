    ...
    <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/70 backdrop-blur-sm">
      <motion.div initial={{ y: '100%' }} animate={{ y: 0 }} exit={{ y: '100%' }}
        ...
        <div className="flex items-center justify-between p-5 border-b border-gray-800 sticky top-0 bg-gray-900 z-10">
          <h2 className="text-lg font-black text-white">Edit Community ✏️</h2>
        ...
        <div className="p-5 space-y-5">
          ...
          {/* Theme Color */}
          <div>
            <label className="block text-gray-400 text-sm mb-2">Theme Color</label>
            ...
            <label className="block cursor-pointer">
              {data.cover_image
                ? <div className="relative h-32 rounded-xl overflow-hidden">
                    ...
                    <div className="absolute inset-0 bg-black/40 flex items-center justify-center"><span className="text-white text-sm">Change 📸</span></div>
                  </div>
                : <div className="h-32 rounded-xl border-2 border-dashed border-gray-700 flex items-center justify-center gap-2">
                    {uploadingCover ? <Loader2 className="w-5 h-5 animate-spin" style={{ color: tc }} /> : <><ImageIcon className="w-6 h-6 text-gray-600" /><span className="text-gray-500 text-sm">Cover photo</span></>}
                  </div>}
          ...
          {/* Background */}
          <div>
            <label className="block text-gray-400 text-sm mb-2">Background Image</label>
            <label className="block cursor-pointer">
              {data.background_image
                ...
                : <div className="h-20 rounded-xl border-2 border-dashed border-gray-700 flex items-center justify-center gap-2">
                    {uploadingBg ? <Loader2 className="w-4 h-4 animate-spin" style={{ color: tc }} /> : <><ImageIcon className="w-5 h-5 text-gray-600" /><span className="text-gray-500 text-xs">Background texture</span></>}
                  </div>}
          ...
          <div className="flex items-center justify-between p-4 rounded-2xl bg-gray-800 border border-gray-700">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-white font-semibold text-sm">Chat Access</p>
                <p className="text-gray-400 text-xs">{data.chat_locked ? 'Members only' : 'Open to all'}</p>
              </div>
            </div>
          ...
          {/* Plan policy */}
          <div>
            <label className="block text-gray-400 text-sm mb-2">Plan Creation Policy</label>
            {[
              { key: 'anyone', emoji: '🌍', title: 'Anyone' },
              { key: 'approval_required', emoji: '✅', title: 'Needs Approval' },
              { key: 'admins_only', emoji: '👑', title: 'Admins Only' },
            ].map((opt) => (
