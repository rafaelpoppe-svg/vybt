      ...
      <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-4">
        ...
        >
          ...
          <div className="text-center mb-6">
            ...
            <h2 className="text-2xl font-bold text-white mb-2">{t.blockUserTitle.replace('{name}', userName)}</h2>
            <p className="text-gray-400 text-sm">{t.blockUserDesc}</p>
          </div>
          ...
          <div className="flex gap-3">
            ...
            <Button onClick={onBlock} disabled={isLoading} className="flex-1 bg-red-600 hover:bg-red-700 text-white">
              {isLoading ? <Loader2 className="w-5 h-5 animate-spin" /> : t.blockUser}
            </Button>
          </div>
        </motion.div>
      </div>
