import { useMemo } from 'react';

// Map party tags to related vibes for cross-matching (new vibe system)
const tagToVibeMap = {
  'Nightclub':    ['Hype', 'Wild', 'Intense', 'Late Night', 'Party Starter'],
  'Bar Crawl':    ['Hype', 'Explorer', 'Party Starter', 'City Night'],
  'House Party':  ['Hype', 'Wild', 'Party Starter', 'Chaos Mode'],
  'Rooftop Party':['Hype', 'VIP Energy', 'City Night', 'Rooftop Energy'],
  'Pool Party':   ['Hype', 'Wild', 'Beach Vibes', 'Festival Mood'],
  'Boat Party':   ['Hype', 'VIP Energy', 'Beach Vibes', 'Festival Mood'],
  'Festival':     ['Festival Mood', 'Hype', 'Wild', 'Intense', 'Party Starter'],
  'Rave':         ['Intense', 'Wild', 'Chaos Mode', 'Late Night'],
  'DJ Session':   ['Hype', 'Festival Mood', 'Late Night', 'Party Starter'],
  'Silent Disco': ['Balanced', 'Hype', 'Explorer', 'Festival Mood'],
  'Live Music':   ['Balanced', 'Explorer', 'Festival Mood', 'Chill'],
  'Erasmus Party':['Hype', 'Explorer', 'Party Starter', 'Flirty'],
  'Student Party':['Hype', 'Wild', 'Party Starter', 'Chaos Mode'],
  'Birthday Party':['Hype', 'Balanced', 'Party Starter', 'Festival Mood'],
  'Themed Party': ['Hype', 'Explorer', 'Festival Mood', 'Party Starter'],
  'After Party':  ['Intense', 'Wild', 'Late Night', 'Chaos Mode'],
  'Pre-Drinks':   ['Balanced', 'Hype', 'Party Starter', 'Flirty'],
  'Chill Night':  ['Chill', 'Lowkey', 'Observer', 'Balanced'],
  'VIP Night Out':['VIP Energy', 'Hype', 'Rooftop Energy', 'City Night'],
  'Beach Club':   ['Beach Vibes', 'Hype', 'Balanced', 'Flirty'],
  'Open Air Party':['Festival Mood', 'Hype', 'Explorer', 'Wild'],
  'Karaoke Night':['Hype', 'Balanced', 'Party Starter', 'Explorer'],
  'BBQ Party':    ['Balanced', 'Chill', 'Explorer', 'Beach Vibes'],
  'Dinner Party': ['Chill', 'Lowkey', 'VIP Energy', 'Observer'],
  'Baile Funk':   ['Wild', 'Hype', 'Party Starter', 'Festival Mood'],
  'Flirt Night':  ['Flirty', 'Hype', 'Balanced', 'City Night'],
  'Coffee Night': ['Chill', 'Lowkey', 'Observer', 'Balanced'],
  'Beach Camp':   ['Beach Vibes', 'Chill', 'Explorer', 'Lowkey'],
  'City Walk':    ['Explorer', 'Chill', 'Observer', 'City Night'],
  'Arcade Night': ['Balanced', 'Explorer', 'Party Starter', 'Hype'],
  'Pool Session': ['Beach Vibes', 'Chill', 'Balanced', 'Flirty'],
  'Beach Session':['Beach Vibes', 'Chill', 'Explorer', 'Balanced'],
  // Legacy
  'Bar Hopping':  ['Hype', 'Explorer', 'Party Starter', 'City Night'],
  'Pub Crawl':    ['Hype', 'Explorer', 'Party Starter', 'City Night'],
  'DJ Set':       ['Hype', 'Festival Mood', 'Late Night', 'Party Starter'],
  'Social Mixer': ['Balanced', 'Flirty', 'Explorer', 'Observer'],
};

function calculateVibeMatch(planTags = [], userVibes = []) {
  if (!planTags.length || !userVibes.length) return 0;
  
  let matchCount = 0;
  
  for (const tag of planTags) {
    if (userVibes.some(vibe => tag.toLowerCase().includes(vibe.toLowerCase()))) {
      matchCount += 2;
      continue;
    }
    const relatedVibes = tagToVibeMap[tag] || [];
    if (relatedVibes.some(vibe => userVibes.includes(vibe))) {
      matchCount += 1.5;
    }
  }
  
  if (userVibes.includes('Curious to every style')) {
    matchCount += 1;
  }
  
  return Math.min(matchCount / planTags.length * 50, 50);
}

function calculatePartyTypeMatch(planTags = [], userPartyTypes = []) {
  if (!planTags.length || !userPartyTypes.length) return 0;
  
  let matchCount = 0;
  
  for (const partyType of userPartyTypes) {
    if (planTags.some(tag => 
      tag.toLowerCase() === partyType.toLowerCase() ||
      tag.toLowerCase().includes(partyType.toLowerCase()) ||
      partyType.toLowerCase().includes(tag.toLowerCase())
    )) {
      matchCount++;
    }
  }
  
  return (matchCount / userPartyTypes.length) * 30;
}

function calculateFriendScore(planId, friendIds = [], allParticipants = []) {
  if (!friendIds.length) return 0;
  
  const planParticipants = allParticipants.filter(p => p.plan_id === planId);
  const friendsGoing = planParticipants.filter(p => friendIds.includes(p.user_id));
  
  if (friendsGoing.length === 0) return 0;
  if (friendsGoing.length >= 3) return 15;
  if (friendsGoing.length >= 2) return 10;
  return 5;
}

function calculatePastAttendanceScore(planTags = [], pastPlans = []) {
  if (!pastPlans.length || !planTags.length) return 0;
  
  const pastTags = pastPlans.flatMap(p => p.tags || []);
  const tagFrequency = {};
  
  for (const tag of pastTags) {
    tagFrequency[tag] = (tagFrequency[tag] || 0) + 1;
  }
  
  let score = 0;
  for (const tag of planTags) {
    if (tagFrequency[tag]) {
      score += Math.min(tagFrequency[tag] * 2, 5);
    }
  }
  
  return Math.min(score, 15);
}

function calculateLocationScore(planCity, userCity) {
  if (!planCity || !userCity) return 0;
  if (planCity.toLowerCase() === userCity.toLowerCase()) return 10;
  return 0;
}

export function useRecommendations({
  plans = [],
  userProfile = {},
  friendIds = [],
  pastPlanIds = [],
  allParticipants = [],
  allPlans = []
}) {
  return useMemo(() => {
    if (!plans.length) return [];
    
    const pastPlans = allPlans.filter(p => pastPlanIds.includes(p.id));
    
    const scoredPlans = plans.map(plan => {
      const scores = {
        vibes: calculateVibeMatch(plan.tags, userProfile?.vibes),
        partyType: calculatePartyTypeMatch(plan.tags, userProfile?.party_types),
        friends: calculateFriendScore(plan.id, friendIds, allParticipants),
        pastAttendance: calculatePastAttendanceScore(plan.tags, pastPlans),
        location: calculateLocationScore(plan.city, userProfile?.city)
      };
      
      const highlightBonus = plan.is_highlighted ? 5 : 0;
      const popularityBonus = Math.min((plan.view_count || 0) / 100, 5);
      
      const totalScore = 
        scores.vibes + 
        scores.partyType + 
        scores.friends + 
        scores.pastAttendance + 
        scores.location +
        highlightBonus +
        popularityBonus;
      
      const reasons = [];
      if (scores.vibes > 20) reasons.push('vibes');
      if (scores.partyType > 15) reasons.push('party_type');
      if (scores.friends > 0) reasons.push('friends');
      if (scores.location > 0) reasons.push('location');
      
      return {
        ...plan,
        matchScore: Math.round(totalScore),
        matchReasons: reasons,
        scoreBreakdown: scores
      };
    });
    
    return scoredPlans.sort((a, b) => b.matchScore - a.matchScore);
  }, [plans, userProfile, friendIds, pastPlanIds, allParticipants, allPlans]);
}